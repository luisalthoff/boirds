...
function valid(poi) {
  return poi && !poi.isDeleted && Number(poi.speed) >= 30 &&
...
async function fetchPage(offset) {
  const params = new URLSearchParams({
    ...
    showNormal: "true", showInvalidLocation: "false",
    showNoDirectionDual: "false", showNoDirectionSingle: "false",
    showDeleted: "false", north: String(BOUNDS.north), south: String(BOUNDS.south),
    east: String(BOUNDS.east), west: String(BOUNDS.west), types: TYPES
  });

  const response = await fetch(`${API}?${params}`, {
    headers: { Accept: "application/json, text/plain, */*", Referer: "https://mapa.maparadar.com/" }
  });
...
async function main() {
  ...
  while (true) {
    ...
    for (const poi of pois) if (valid(poi)) radars.push(compact(poi));
    offset += pois.length;
  ...
  const output = { updatedAt: new Date().toISOString(), radars };
